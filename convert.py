...
import openpyxl
from openpyxl.styles import Border, Side, Color, Font, numbers
...
class makeFile:
    ...
    def makeEXCEl(self, data, excel_dir, sample_path, last_index, pdf_dir):
        if makeFile.flag == 0:
            count = 0
        elif makeFile.end_time == last_index:
            if str(data['14.入出庫作業開始日'])[0:7] != makeFile.contract_date or (str(data['14.入出庫作業開始日'])[0:7] == makeFile.contract_date and data['15.会社名'] != makeFile.global_company):
                print(makeFile.global_factory)
                excel_data = openpyxl.load_workbook(sample_path)
                ws = excel_data.active
                ws['D11'] = makeFile.d11
                ws['D12'] = makeFile.d12
                ws['C14'] = makeFile.c14
                ws['C14'].alignment = Alignment(horizontal='right')
                ws['E11'] = ''

                ws['N29'] = makeFile.n29
                ws['N29'].alignment = Alignment(horizontal='right')
                ws['D33'] = makeFile.d33
                ws['E33'] = makeFile.e33
                ws['F33'] = makeFile.f33
                ws['G33'] = makeFile.g33
                ...
                for y in range(len(makeFile.navid_array)):
                    ...
                    if y > 8:
                        ...
                        ws['M' + str(makeFile.point) ]._style = ws['M16']._style
                        ws['N' + str(makeFile.point) ]._style = ws['N16']._style
                        ws['C' + str(makeFile.point) ]._style = ws['C16']._style
                        ws['E' + str(makeFile.point) ]._style = ws['E16']._style
                        ws['F' + str(makeFile.point) ]._style = ws['F16']._style
                        ws['K' + str(makeFile.point) ]._style = ws['K16']._style
                    ...
                    ws['K' + str(makeFile.point) ].number_format = numbers.FORMAT_NUMBER
                    if makeFile.pay1_array[y] < 0:
                        _pay_cell = ws['M' + str(makeFile.point)]
                        _pay_cell.font = Font(color='FFFF0000')
                    else:
                        _pay_cell = ws['M' + str(makeFile.point)]
                        _pay_cell.font = Font(color='FF000000')
                    ...
                    ws['M' + str(makeFile.point) ].number_format = numbers.FORMAT_NUMBER
                    ws['N' + str(makeFile.point) ].number_format = numbers.FORMAT_NUMBER
                ...
                ws['M' + str(makeFile.point + 1) ].number_format = numbers.FORMAT_NUMBER
                ws['N' + str(makeFile.point + 1) ].number_format = numbers.FORMAT_NUMBER
            ...
            makeFile.c14 = str(datetime.strptime(str(data['14.入出庫作業開始日']), '%Y-%m-%d %H:%M:%S').strftime('%m/%d/%Y %H:%M:%S'))[0:10]
            makeFile.n29 = str(datetime.strptime(str(data['支払期限']), '%Y-%m-%d %H:%M:%S').strftime('%m/%d/%Y %H:%M:%S'))[0:10]
            ...
            makeFile.start_array.append(str(datetime.strptime(str(data['14.入出庫作業開始日']), '%Y-%m-%d %H:%M:%S').strftime('%m/%d/%Y %H:%M:%S'))[0:10])
            ...
            makeFile.global_factory = data['取引先口座管理']
            makeFile.pay2_array.append(data['M.総支払額(税込)'])
            makeFile.file_name = str(data['V支払グループ']).split('-')[0] + '-' + str(data['14.入出庫作業開始日'])[0:7]
            ...
            work_sheets.ExportAsFixedFormat(0, real_name)
            excel.Quit()
        elif str(data['14.入出庫作業開始日'])[0:7] != makeFile.contract_date or (str(data['14.入出庫作業開始日'])[0:7] == makeFile.contract_date and data['15.会社名'] != makeFile.global_company):
            print(makeFile.global_factory)
            excel_data = openpyxl.load_workbook(sample_path)
            ws = excel_data.active
            ws['D11'] = makeFile.d11
        ...
        makeFile.global_factory = data['取引先口座管理']
        makeFile.contract_date = str(data['14.入出庫作業開始日'])[0:7]
        makeFile.flag = 1
        makeFile.end_time = makeFile.end_time + 1

    def intWithCommas(self, x):
        if x < 0:
            return '-' + makeFile.intWithCommas(-x)
        result = ''
        while x >= 1000:
            x, r = divmod(x, 1000)
    ...
    while True:
        ...
        if event == '自動明細書生成':
            ...
            for x in range(len(pay_data)):
                if pd.isna(pay_data.iloc[x]['14.入出庫作業開始日']):
                    count = 0
                else:
                    temp_data.append(pay_data.iloc[x])
            temp_data.sort(key=lambda x: (x.iloc[5], x.iloc[3], x.iloc[6]), reverse=False)

            for x in range(len(temp_data)):
                makeFile.makeEXCEl(temp_data[x], excel_dir, sample_path, len(temp_data), pdf_dir)
